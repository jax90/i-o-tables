...
pooled_price_data =
  ...
  select(country = `Country or Area`,
         ...
         currency = Currency) %>%
  mutate(current_value = un_price_data_current$Value[match(paste0(year,item,country,currency),
...
get_constant_figaro_tables = function(years = 2010:2021,base = 2020,verbose = T,folder = NULL)
{
