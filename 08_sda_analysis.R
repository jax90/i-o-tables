...
sectoral_sda_analysis = function(results,verbose = T)
{
  time_span = sort(unique(results$time_period))
  ...
  for(year in as.numeric(time_span[-1]))
  {
    ...
    e0 = results %>%
      filter(time_period == (year-1)) %>%
      separate(resource_id,into = c('country','industry'),sep = "_",extra = 'merge') %>%
      mutate(embodied_emissions = rowSums(across(matches("embodied_emissions"))),
             industry = case_when(
      ...
      group_by(industry) %>%
      summarise(value = sum(embodied_emissions,na.rm=T))
    ...
    #For sectoral footprint, one has to nullify non-target L columns

    for(sector in c("Hardware","Communications","IT Services","Mediated via other industries"))
    {
      if(sector == 'Hardware')
      {
        L0Work = L0 ; L0Work[,!grepl("C26",colnames(L0Work))] = 0
        L1Work = L1 ; L1Work[,!grepl("C26",colnames(L1Work))] = 0
        e0Work = e0  %>% filter(industry == "hardware") %>% select(value) %>% as.numeric()
        e1Work = e1  %>% filter(industry == "hardware") %>% select(value) %>% as.numeric()
      }
      if(sector == 'Communications')
      {
        L0Work = L0 ; L0Work[,!grepl("J61",colnames(L0Work))] = 0
        L1Work = L1 ; L1Work[,!grepl("J61",colnames(L1Work))] = 0
        e0Work = e0  %>% filter(industry == "communications") %>% select(value) %>% as.numeric()
        e1Work = e1  %>% filter(industry == "communications") %>% select(value) %>% as.numeric()
      }
      if(sector == 'IT Services')
      {
        L0Work = L0 ; L0Work[,!grepl("J62_63",colnames(L0Work))] = 0
        L1Work = L1 ; L1Work[,!grepl("J62_63",colnames(L1Work))] = 0
        e0Work = e0  %>% filter(industry == "IT services") %>% select(value) %>% as.numeric()
        e1Work = e1  %>% filter(industry == "IT services") %>% select(value) %>% as.numeric()
      }
      if(sector == 'Mediated via other industries')
      {
        L0Work  = L0 ; L0Work[,grepl("C26|J61|J62",colnames(L0Work))] = 0
        L1Work  = L1 ; L1Work[,grepl("C26|J61|J62",colnames(L1Work))] = 0
        e0Work = e0  %>% filter(industry == "mediated via other industries") %>% select(value) %>% as.numeric()
        e1Work = e1  %>% filter(industry == "mediated via other industries") %>% select(value) %>% as.numeric()
      }

    dif_B = (1/2) * t(b1 - b0) %*% L1Work %*% u1 %*% Y1 + (1/2) * t(b1 - b0) %*% L0Work %*% u0 %*% Y0

    dif_L = (1/2) * t(b0) %*% (L1Work - L0Work) %*% u1 %*% Y1 + (1/2) * t(b1) %*% (L1Work - L0Work) %*% u0 %*% Y0

    dif_u = (1/2) * t(b0) %*% L0Work %*% (u1 - u0) %*% Y1 + (1/2) * t(b1) %*% L1Work %*% (u1 - u0) %*% Y0

    dif_Y = (1/2) * t(b0) %*% L0Work %*% u0 %*% (Y1 - Y0) + (1/2) * t(b1) %*% L1Work %*% u1 %*% (Y1 - Y0)

    if((round(dif_B + dif_L + dif_u + dif_Y) != round(sum(e1Work,na.rm=T) - sum(e0Work,na.rm=T))) & verbose) print(paste0('Error detected :',year)) #IT WORKS

    sda_year = data.frame(total_prod = sum(e0Work,na.rm=T),
                          direct_em = as.numeric(dif_B),
                          eco_tech = as.numeric(dif_L),
                          fd_vol = as.numeric(dif_Y),
                          fd_str = as.numeric(dif_u),
...
sectoral_sda_results = sto_sec = sectoral_sda_analysis(results,T)


sectoral_sda_results = sto_sec %>%
  group_by(sector) %>%
  mutate(id = seq(2010,2020.8,1/5),
         amount_share = case_when(amount_share == 100 ~ "",
                                  amount_share < 0 ~ paste0("- ",abs(amount_share)," %"),
                                  T ~ paste0("+ ",amount_share," %")),
         year_changes = case_when(year_changes < 0 ~ paste0("- ",round(abs(year_changes / 1000))),
         ...
         min_end = min(End),
         max_end = max(End),
         scale_y_bot = min_end * 0.85,
         scale_y_top = max_end * 1.125) %>%
  ungroup()

sto_gg = list()
for(i in c("Hardware","Communications","IT Services","Mediated via other industries"))
{

sto_gg[[i]] = ggplot(sectoral_sda_results %>% filter(sector == i), aes(time_period, fill = Contributions)) + geom_rect(aes(
  xmin = id, xmax = id + 1/5, ymin = End,
  ymax = Start)) +
  scale_fill_manual(breaks = c("Direct emissions intensities","Technology changes","Final demand : volume","Final demand : basket"),
                    values = c("#CC9900","#9900CC","#FF0000","#0066CC",'#333333'))+
  ...
  coord_cartesian(ylim = c(limits = c(sectoral_sda_results %>% filter(sector == i) %>% select(scale_y_bot,scale_y_top) %>% distinct() %>% as.numeric())),
                  xlim = c(limits = c(2010,max(sectoral_sda_results$time_period))),
                  clip = 'on') +
  geom_text(aes(label=amount_share,y = pmax(Start,End)*1.05,x = id+1/12,angle = 90),size = 2.5) +
  # geom_segment(data = sectoral_sda_results %>% filter(time_period != max(time_period) & sector == i),aes(x = time_period - 4/5,
  #                                                                                          y = 1.025 * sectoral_sda_results %>% filter(sector == i) %>% select(scale_y_bot) %>% unique() %>% as.numeric(),
  #                                                                                          xend = time_period - 0.05,
  #                                                                                          yend = 1.025 * sectoral_sda_results %>% filter(sector == i) %>% select(scale_y_bot) %>% unique() %>% as.numeric()),
  #              arrow = arrow(length = unit(0.2, "cm")),linewidth = .8) +
  # geom_text(data = sectoral_sda_results %>% filter(time_period != max(time_period) & sector == i),aes(label = year_changes,y = 1.05 * sectoral_sda_results %>% filter(sector == i) %>% select(scale_y_bot) %>% unique() %>% as.numeric(),x = time_period - 0.4),size = 3.5) +
  labs(title = i) +
  theme(plot.title = element_text(size = 14,face = 'bold',hjust = .5))
}

ggpubr::ggarrange(
  sto_gg$Hardware, sto_gg$Communications,sto_gg$`IT Services`,sto_gg$`Mediated via other industries`,
